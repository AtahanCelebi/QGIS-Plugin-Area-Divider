...
#            self.sr = self.layer.GetSpatialRef().ExportToProj4()
    ...
    def apply_graduated_symbology(self,area_size):
        ...
        myRangeList = []
        number = self.dlg.lineEdit_2.text()


        symbol = QgsSymbol.defaultSymbol(self.new2.geometryType())
        symbol.setColor(QColor("#205E14"))
        myRange = QgsRendererRange(0, area_size, symbol, "%s'den kucuk alanlar"%(area_size))
        myRangeList.append(myRange)

        symbol = QgsSymbol.defaultSymbol(self.new2.geometryType())
        symbol.setColor(QColor("#F92525"))
        myRange = QgsRendererRange(area_size, 9999999999999999, symbol, "%s'den buyuk alanlar"%(area_size))
        myRangeList.append(myRange)

        myRenderer = QgsGraduatedSymbolRenderer('Area', myRangeList)
        myRenderer.setMode(QgsGraduatedSymbolRenderer.Custom)

        self.new2.setRenderer(myRenderer)

        print("Graduated color scheme applied")


    def error_msj(self, uyari):
        QMessageBox.warning(self.dlg.show(), self.tr("Uyarı!!!"), self.tr(str(uyari)), QMessageBox.Ok)
    def run(self):
        ...
        if result:
            ...
            if type_of_layer =="line":
                ...
                oznitelikler = self.v_layer.fields().names()
                ...
                if len(lines) > 0:
                    ...
                    for cizgi in lines:
                        ...
                        self.v_layer.updateFeature(cizgi)
                ...
                self.error_msj(" Lütfen seçtiğiniz shapefile dosyasını kontrol ediniz !")
